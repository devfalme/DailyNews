...
- (WKWebView *)webView {
    ...
    if (!_webView) {
        ...
        CGRect frame = CGRectMake(0, y, S_Width, S_Height - (IsIphoneX?88:64) - Bottom_Height);
...
- (UIView *)bottomView {
  ...
  //后题，前进，首页，刷新，菜单（菜单里有“分享”+“清除缓存”）
        for (NSInteger i = 0; i < cates.count; i ++) {
            ...
            UIButton *flashBtn = [[UIButton alloc] initWithFrame:frame];
            [flashBtn setImage:[UIImage imageNamed:[NSString stringWithFormat:@"webControl%ld",(long)i]] forState:UIControlStateNormal];
            flashBtn.tag = i;
            [[flashBtn rac_signalForControlEvents:UIControlEventTouchUpInside] subscribeNext:^(id x) {
                [self bottomButtonClick:flashBtn.tag];
            }];
